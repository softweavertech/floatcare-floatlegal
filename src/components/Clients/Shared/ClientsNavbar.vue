<template>
    <div class="bg-white overflow-hidden w-100">
        <div class="patient-list-top">
            <div class="d-flex align-items-center p-4 ">
                <h1 class="me-auto">Clients</h1>
                <button v-if="!createPatient" @click="addUserAction" class="btn btn-outline-secondary"
                    >
                    <svg width="24px" height="24px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink">
                        <title>Icons/ic_add_circle</title>
                        <defs>
                            <path
                                d="M12,2 C6.48,2 2,6.48 2,12 C2,17.52 6.48,22 12,22 C17.52,22 22,17.52 22,12 C22,6.48 17.52,2 12,2 L12,2 Z M17,13 L13,13 L13,17 L11,17 L11,13 L7,13 L7,11 L11,11 L11,7 L13,7 L13,11 L17,11 L17,13 L17,13 Z"
                                id="path-1"></path>
                            <rect id="path-3" x="0" y="0" width="50" height="50"></rect>
                        </defs>
                        <g id="Symbol" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g id="Icons/ic_add_circle">
                                <mask id="mask-2" fill="white">
                                    <use xlink:href="#path-1"></use>
                                </mask>
                                <g id="Mask"></g>
                                <g id="🎨-Color" mask="url(#mask-2)">
                                    <g transform="translate(-13.000000, -13.000000)">
                                        <mask id="mask-4" fill="white">
                                            <use xlink:href="#path-3"></use>
                                        </mask>
                                        <use id="Color" stroke="none" fill="#0070FF" fill-rule="evenodd"
                                            xlink:href="#path-3"></use>
                                    </g>
                                </g>
                            </g>
                        </g>
                    </svg> Create Client</button>
            </div>
           
        </div>
        <!-- <router-view></router-view> -->
    </div>
    <hr class="m-0">
    <b-modal
        v-model="searchUserModal"
        centered
        size="960"
        id="searchUserModal "
        class="searchUserModal"
        content-class="search-modal create-modal"
        hide-header
        hide-footer
        no-close-on-backdrop
        :no-enforce-focus="true"
      >
        <searchUserModal
          :pageTitle="pageTitle"
          :SearchData="SearchData"
          :pageHeader="pageHeader"
          :closeSearchModel="closeSearchModel"
          :hideSearchModel="hideSearchModel"
        ></searchUserModal>
      </b-modal>
</template>

<script>

import searchUserModal from "@/components/Clients/Shared/searchUserModal.vue";
export default ({
   
     components: {
    searchUserModal,
  },
    data() {
        return {
             searchUserModal: false,
            displayItem: 'patient',
            breadCrumb: [{
                url: "/patient",
                name: "Patient",
                value: 'patient'
            }
            ],
        };
    },
    methods: {
        addUserAction(){
            this.searchUserModal = true
        },
        tabsClick(value, link) {
            this.displayItem = value
            this.$router.push(link);
        },
         closeSearchModel() {
      this.searchUserModal = false;
    },
    }
})
</script>
<style scope lang="scss">
.patient-list-top {
    padding-bottom: 0px;

}


</style>